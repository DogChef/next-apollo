...
import { styled as matStyled } from "@material-ui/core/styles";
...
import { GET_ARTICLE_MODIFICATIONS } from "./core/queries";
...
const ArticleModifications = ({ id }) => {
  const { data } = useQuery(GET_ARTICLE_MODIFICATIONS, {
    fetchPolicy: "no-cache",
    variables: { id: id }
  });

  const modificationList = data?.articleModifications?.reverse();
